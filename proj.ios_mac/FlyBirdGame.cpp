...
void FlyBirdGame::initUI()
{
	...
	logo->setPosition(Point(winSize.width / 2, winSize.height / 2 + logo->getContentSize().height * 2));
	logo->setTag(TAG_LOGO);
	this->addChild(logo, 1);

	// over logo
	auto overLogo = Sprite::create(bird_gameover);
	overLogo->setPosition(Point(winSize.width / 2, winSize.height / 2 + overLogo->getContentSize().height));
	...
	// score
	auto score = LabelBMFont::create("0", "futura-48.fnt");
